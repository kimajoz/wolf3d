...
/*   Updated: 2017/11/20 15:24:08 by pbillett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/wolf3d.h"

void			w_set_pxl_spr(int bx, int by, t_wind *w)
{
...
void			w_set_pxl_spr_root(int bx, int by, int size, t_wind *w)
{
	...
	while ((by + w->w.pxl_y) < (by + size))
	{
		...
		while ((bx + w->w.pxl_x) < (bx + size))
		{
			...
			w->w.color = getcolor(&w->w.sprite[w->w.sprnumb - 1].img,
			percx * 64, percy * 64);
			if (mlibx_dot_in_window(w, bx + w->w.pxl_x, by + w->w.pxl_y) &&
			w->w.color != 0 && w->w.color != 16843008 && w->w.color)
				w_set_pxl_spr(bx, by, w);
			w->w.pxl_x++;
		}
		w->w.pxl_y++;
	}
...
void			w_calc_render_spr(t_wind *w, t_sprite sprite)
{
	...
	if (w->w.s_xleft < w->width && w->w.s_xleft > -w->width
			&& s_ytop > -w->height && s_ytop < w->height)

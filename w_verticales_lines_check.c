...
void			v_test(t_wind *w, t_dpoint *d, t_dpoint *p, t_dpoint *dist)
{
	while (p->x >= 0 && p->x < w->b.nbr_elem_line[0] && p->y >= 0 && p->y <
		w->b.nbrtot_of_line)
	{
		w->w.wall = (t_dpoint){floor(p->x + (w->w.right ? 0 : -1)), floor(p->y), 0};
		...
		if ((w->b.tab_int[(int)w->w.wall.y][(int)w->w.wall.x] > 0))
		{
			*dist = (t_dpoint){(p->x * MMS) - (w->cam.pos.x * MMS), (p->y * MMS) -
			...
			w->w.hit = *p;
			w->w.texX = (!w->w.right) ? TEXWIDTH - (fmod(p->y, 1) * TEXWIDTH) :
				fmod(p->y, 1) * TEXWIDTH;
...
void			h_test(t_wind *w, t_dpoint *d, t_dpoint *p, t_dpoint *dist)
{
	while (p->x >= 0 && p->x < w->b.nbr_elem_line[0] && p->y >= 0 && p->y <
			w->b.nbrtot_of_line)
	{
		...
		if ((w->b.tab_int[(int)w->w.wall.y][(int)w->w.wall.x] > 0))
		{
			*dist = (t_dpoint){(p->x * MMS) - (w->cam.pos.x * MMS), (p->y * MMS)
				...
				(w->w.block_dist < w->w.dist && !ft_fiszero(w->w.block_dist)))
			{
				...
				w->w.hit = *p;
				w->w.texX = (!w->w.up) ? TEXWIDTH - (fmod(p->x, 1) * TEXWIDTH) :
					fmod(p->x, 1) * TEXWIDTH;
				w->w.textnumb = w->b.tab_int[(int)w->w.wall.y][(int)w->w.wall.x] - 1;

...
void				put_info_cam(t_wind *w, int leftm, int row, int bl)
{
	...
	mlibx_putstring(ft_itoa(w->cam.pos.x), row, leftm + bl, w);
...
void				put_info_game(t_wind *w, int row, int bl)
{
	...
	mlibx_putstring(ft_itoa(w->w.player.health), row, midm + 7, w);
	mlibx_putstring("Ammunition:", row, midm + 12, w);
	mlibx_putstring(ft_itoa(w->w.player.ammunition), row, midm + 24, w);
	...
	mlibx_putstring(ft_itoa(w->w.player.totalscore), row, rightm + bl + 1,
		w);
	row++;
...
void				put_info(t_wind *w)
{
	...
	mlibx_putstring(ft_itoa(w->w.info.raynumb), row, COLUMN, w);
	row = 25;
	mlibx_putstring("Toogles:", row, MARGIN, w);
	row = 26;
	mlibx_putstring("5: Minimap rays", row, MARGIN, w);
	mlibx_putstring(ft_itoa(w->w.info.ray_minimap), row, COLUMN, w);
	row = 27;
	mlibx_putstring("6: Background", row, MARGIN, w);
	mlibx_putstring(ft_itoa(w->w.info.bg), row, COLUMN, w);
	row = 28;
	mlibx_putstring("7: Textures", row, MARGIN, w);
	mlibx_putstring(ft_itoa(w->w.info.texture), row, COLUMN, w);
